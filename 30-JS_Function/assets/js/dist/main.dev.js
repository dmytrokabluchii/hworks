...
var argCalculate = calcArg(6, 3, 4, 7, 34, 999, 'Hello');
console.log(argCalculate); // 3. Напиши функцію, яка приймає 2 числа і повертає :
...
function factorialFirst(n) {
  if (n === 0 || n === 1) return 1;else return n * factorialFirst(n - 1);
}

var rezFactorialFirst = factorialFirst(5);
console.log(rezFactorialFirst); // Альтернативный вариант(2) решения циклом: с запросом числа и выводом результата в alert
...
function changeInOneNumber(one, two, three) {
  var rez = String(one) + two + three;
  return rez = +rez;
}

var rezChangeInOneNumber = changeInOneNumber(1, 4, 9);
console.log(rezChangeInOneNumber); // 6. Напиши функцію, яка приймає довжину і ширину прямокутника і обчислює його площу. Якщо в функцію передали 1 параметр, 
...
function lengthSideRectangle(numOne, numTwo) {
  var rezRectangle = numOne * numTwo;
  var rezSquare = (numOne || numTwo) * (numOne || numTwo);
  ...
  var rez = numOne > 0 && numTwo > 0 ? "The space of rectangle =  ".concat(rezRectangle) : "The space of square =  ".concat(rezSquare);
  return rez;
}

var rezLengthSideRectangle = lengthSideRectangle(5, 3);
console.log(rezLengthSideRectangle); // 7. Напиши функцію, яка перевіряє, чи є передане їй число “досконалим числом”. Досконале число - це число, яке дорівнює сумі всіх своїх дільників.
...
function checkPerfectNumber(num2) {
  ...
  var num = !num2 ? parseInt(document.getElementById('perfect_number').value) : num2;

  if (num === 0) {
    alert(num + ' - not a Perfect Number!'); // document.getElementById("resultsPerfectNumber").innerHTML = num + ' - not a Perfect Number!';
  ...
  var sum = 0;
  var residue; // перебираем циклом от 1 до заданного числа -1
...
function checkPerf(num) {
  ...
  if (num === 0) {
    return num + ' - not a Perfect Number!'; // return false;
  } else if (num === sum) {
    console.log(num + ' - is a Perfect Number!'); // return true;
...
var rezCheckPerfectNumber = checkPerf(496); // console.log(rezCheckPerfectNumber);
// 8. Напиши функцію, яка приймає мінімальне і максимальне значення для діапазону, і виводить тільки ті числа з діапазону, які є досконалими. 
// Використовуй написану раніше функцію, щоб дізнатися, чи є це число досконалим.
// Вариант решения с запросом числа у пользователя и выводом результата в карточке HTML

function findAmountOfPerfectNumbers() {
  var minNumber = parseInt(document.getElementById('perfect_number_in_range_min').value);
  var maxNumber = parseInt(document.getElementById('perfect_number_in_range_max').value);
...
var rezFindAmountOfPerf = findAmountOfPerf(4, 940); // console.log(rezFindAmountOfPerf);
...
first(second); // Применение CSS стилей в консоли
