...
function factorial(n) {
  if (+n === 0 || +n === 1) return results_factorial.innerText = 1;else if (+n >= 20) return results_factorial.innerText = false;else return results_factorial.innerText = +n * factorial(+n - 1);
}
...
function changeInOneNumber(one, two, three) {
  if (one >= 10 || two >= 10 || three >= 10) {
    return results_changeInOneNumber.innerText = false;
  } else {
    return results_changeInOneNumber.innerText = one * 100 + two * 10 + three;
  }
}
...
function lengthSideRectangle(length, width) {
  var rezRectangle;
  var rezSquare; // if (isNaN(length, width)) {return false;}
...
function checkPerfectNumber(num) {
  ...
  if (num === 0) {
    return results_checkPerfectNumber.innerText = false;
  } else if (num === sum) {
    return results_checkPerfectNumber.innerText = true;
  } else {
    return results_checkPerfectNumber.innerText = false;
  }
...
function checkPerfectNumberRange() {
  var rangeStart = parseInt(document.getElementById('perfect_number_in_range_min').value),
  ...
  var rez = findAmountOfPerfectNumbers(rangeStart, rangeEnd);
